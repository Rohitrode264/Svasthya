    ...
    try {
        ...
        if (medication.user.email) {
            await sendMail(
                ...
                "Medication Schedule Created - MediMinder",
                `A schedule for "${medication.name}" has been created: ${scheduleSummary}.`,
                "MediMinder"
            );
        }
...
export const updateSchedule = async (req: Request, res: Response) => {
    try {
        ...
        const { timeOfDay, recurrence } = req.body as {
            timeOfDay?: string;
            recurrence?: string;
        };
...
async function generateRemindersForSchedule(scheduleId: string, medicationId: string, timeOfDay: string) {
    const reminders = [];
    const now = new Date();
...
async function regenerateRemindersForSchedule(scheduleId: string) {
