...
export const logDose = async (req: Request, res: Response) => {
    try {
        ...
        const validStatuses: DoseStatus[] = [DoseStatus.TAKEN, DoseStatus.MISSED, DoseStatus.SKIPPED];
        if (!validStatuses.includes(normalizedStatus as DoseStatus)) {
            return res.status(400).json({
                ...
                if (medication.refillThreshold &&
                    ...
                    if (medication.user.email) {
                        await sendMail(
                            ...
                            "Medication Refill Reminder - MediMinder",
                            `Your medication "${medication.name}" is running low. Current quantity: ${medication.quantity - 1}. Please consider refilling soon.`,
                            "MediMinder"
                        );
                    }
                }
...
export const getMedicationDoseLogs = async (req: Request, res: Response) => {
    try {
        ...
        let whereClause: any = { medicationId };
...
export const getUserDoseLogs = async (req: Request, res: Response) => {
    try {
...
export const getDoseStatistics = async (req: Request, res: Response) => {
    try {
        const { userId } = req.params;
