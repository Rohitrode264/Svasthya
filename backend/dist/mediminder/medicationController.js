...
export const addMedication = async (req, res) => {
    try {
        const { userId, name, brand, strength, quantity, refillThreshold, instructions, scheduleTimes } = req.body;
        if (!userId || !name) {
        ...
        if (user.email) {
            const scheduleText = created.schedules.length > 0
      ...
      `;
            await sendMail(user.email, `Medication Added - ${created.name}${strengthText}`, emailHtml, "MediMinder");
        }
...
export const logDose = async (req, res) => {
    try {
        ...
        if (![DoseStatus.TAKEN, DoseStatus.MISSED, DoseStatus.SKIPPED].includes(normalizedStatus)) {
            return res
                .status(400)
        ...
        if (normalizedStatus === DoseStatus.TAKEN) {
            if (typeof medication.quantity === "number" && medication.quantity > 0) {
                ...
                const threshold = typeof updated?.refillThreshold === "number" ? updated.refillThreshold : null;
                if (updatedQty !== null &&
                    threshold !== null &&
                    updatedQty <= threshold &&
                    medication.user?.email) {
                    const strengthText = medication.strength ? ` ${medication.strength}` : "";
                    await sendMail(medication.user.email, `Refill Reminder - ${medication.name}${strengthText}`, `You’re running low on <strong>${medication.name}${strengthText}</strong> — only ${updatedQty} left! ⏰`, "MediMinder");
                }
            }
        }
 ...
 */
export const getMedicationsForUser = async (req, res) => {
    try {
