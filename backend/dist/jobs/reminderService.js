...
cron.schedule("* * * * *", async () => {
    try {
        ...
        console.log(`🕐 [UTC] Checking schedules for ${hhmmUTC} between ${minuteStart.toISOString()} and ${minuteEnd.toISOString()}`);
        ...
        `;
            try {
                await sendMail(user.email, `Medication Reminder: ${medication.name}`, emailBody, "MediMinder");
                ...
                await prisma.alert.create({
                    data: {
                        userId: user.id,
                        type: "MED_REMINDER",
                        message: `Reminder sent for ${medication.name} (UTC ${DateTime.fromJSDate(reminder.scheduledAt)
                            .toUTC()
                            .toFormat("HH:mm")})`,
                        delivered: true,
                    },
...
cron.schedule("0 9 * * *", async () => {
    try {
        ...
        for (const med of meds) {
            ...
            const threshold = typeof med.refillThreshold === "number" ? med.refillThreshold : 5;
            if (med.user?.email && currentQty <= threshold) {
                await sendMail(med.user.email, "Refill Reminder - MediMinder", `Your "${med.name}" is running low. Only ${currentQty} left. Please refill soon.`, "MediMinder");
                await prisma.alert
                    .create({
                    data: {
                        userId: med.user.id,
